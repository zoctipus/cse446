...
def vanilla_solution(x: Vector, y: Vector, A: Matrix, B: Matrix) -> Vector:
    ...
    # (A + A^T)x
    term1 = [a + b for a, b in zip(vanilla_matmul(A, x), vanilla_matmul(vanilla_transpose(A), x))]
    # B^t * y
    term2 = vanilla_matmul(vanilla_transpose(B), y)

    return [a + b for a, b in zip(term1, term2)]
...
def numpy_solution(
    ...
    """
    # 2Ax
    term1 = (A + np.transpose(A)) @ x
    term2 = np.dot(np.transpose(B), y)
    return term1 + term2
...
def main():
    ...
    """
    RNG = np.random.RandomState(seed=446)  # pylint: disable=no-member
